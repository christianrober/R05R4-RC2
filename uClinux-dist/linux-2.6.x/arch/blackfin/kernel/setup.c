 ...
 * Description:
 *
 * Rev:          $Id: setup.c,v 1.75 2005/12/21 08:05:54 sonicz Exp $
 *
 * Modified:
...
void __init setup_arch(char **cmdline_p)
{
	...
	*cmdline_p = &command_line[0];
	...
	/* Reserve 96MB for audio storage */
	_ramend = (32 * 1024 * 1024);
...
/* Due to a Hardware Anomaly we need to limit the size of usable instruction memory to max 60MB */
	...
	printk("about to initialize cache.\n");
		...
		unsigned long block_size, unsigned long CPLB_data)
{
	...
	while (start < end) {
	    	if (table == dcplb_table)
		{
		    printk("DCPLB: %08lx len %08lx %s\n", start, block_size, 
			   (CPLB_data & 0x1000) ? "CACHE" : "NON-CACHE");
		}

	    	else if (table == dpdt_table)
		{
		    printk("DPDT: %08lx len %08lx %s\n", start, block_size, 
			   (CPLB_data & 0x1000) ? "CACHE" : "NON-CACHE");
		}
...
# endif
	...
	pos =
	    fill_cpl_tables(dcplb_table, pos, SIZE_4M, _ramend - SIZE_4M,
			    SIZE_4M, SDRAM_DGENERIC);
	pos =
	    fill_cpl_tables(dcplb_table, pos, _ramend, 
			    _ramend + (16 - pos/2) * SIZE_4M,
			    SIZE_4M, SDRAM_DGENERIC);
	while (pos < 32)
		dcplb_table[pos++] = 0;
...
/*TODO: Make mtd none cachable in L1 */
	pos =
	    fill_cpl_tables(dpdt_table, pos, _ramend - SIZE_1M, _ramend,
			    SIZE_1M, SDRAM_DNON_CHBL);
	pos =
	    fill_cpl_tables(dpdt_table, pos, _ramend, RAM_END, SIZE_4M, 
			    SDRAM_DGENERIC);
#if defined (CONFIG_BF561)
# if defined (CONFIG_BFIN561_EZKIT)
	pos =
